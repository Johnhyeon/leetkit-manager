...
- **증권사 연결** — StockLens 분봉용 증권사 Open API 연결(한국투자증권·키움증권,
  시세 전용 App Key, 저장 전 연결 시험, 실전·모의 환경 관리, 환경별·전체 연결
  해제). **하나만 연결하면 충분**하고, 여러 곳을 연결해도 분석 데이터는 주 사용
  증권사 한 곳에서만 가져옴(자동 전환 없음). 연결 창 오른쪽 위 물음표로 증권사별
  지원 범위와 IP 제한 차이를 확인. 키는 명령행이나 파일이 아니라 OS 자격 증명
  저장소로만 전달되고, 연결 해제는 프로그램·라이선스·MCP 등록을 건드리지 않음
